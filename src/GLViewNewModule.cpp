...
void GLViewNewModule::updateWorld()
{
    ...
    if (cam->getPosition().x > 411 && cam->getPosition().x < 1130 && cam->getPosition().y > -1444 && cam->getPosition().y < -744
        && cam->getPosition().z > -45 && cam->getPosition().z < 700 && firstCheck == false && theGUI->onGoing == true)
    {
        firstCheck = true;
    }
...
void Aftr::GLViewNewModule::loadMap()
{
   ...
   std::string shinyRedPlasticCube( ManagerEnvironmentConfiguration::getLMM() + "/models/cube4x4x4redShinyPlastic_pp.wrl" );
   std::string jet1(ManagerEnvironmentConfiguration::getLMM() + "/models/jet_wheels_down_PP.wrl");
   std::string spaceStation(ManagerEnvironmentConfiguration::getLMM() + "/models/spaceStation220x130x160.wrl");
   std::string spaceShip1(ManagerEnvironmentConfiguration::getLMM() + "/models/spaceShipSmall.wrl");
